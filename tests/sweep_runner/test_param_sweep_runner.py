...
def test_execute_before_sweep_modifies_block(mocker: MockerFixture):
    ...
    # Define pipeline, stages, runner objects, and execute the methods in the stage before the
    # sweep
    #
    # NOTE: the pipeline object is only needed for providing the loader, and the methods needed
    # for the purpose of this test are only the ones in the "before sweep" stage, which is why
    # the pipeline and stages object both have only partial pipeline information
    pipeline = Pipeline(loader=loader, methods=[])
    stages = Stages(
        before_sweep=[m1, m2],
        sweep=[],
