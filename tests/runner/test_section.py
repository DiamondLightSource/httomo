...
def test_sectionizer_splits_section_if_multiple_padding_methods(mocker: MockerFixture):
    p = Pipeline(
        loader=make_test_loader(mocker, pattern=Pattern.projection),
        methods=[make_test_method(mocker, pattern=Pattern.projection, padding=False),
    ...
    assert len(s[0]) == 3  # loader + 3 methods, as we want right before the next padding method
