...
def test_get_method_names_for_snapshot_saver(
    ...
    method3 = make_test_method(mocker, method_name="data_checker", pattern=Pattern.projection)
    method4 = make_test_method(mocker, method_name="find_center_pc", pattern=Pattern.sinogram)
    method5 = make_test_method(mocker, method_name="m5_rec", pattern=Pattern.sinogram)
    method6 = make_test_method(mocker, method_name="data_checker", pattern=Pattern.sinogram)
    method7 = make_test_method(mocker, method_name="m7_rec", pattern=Pattern.projection)
    method8 = make_test_method(mocker, method_name="data_checker", pattern=Pattern.projection)
    method9 = make_test_method(mocker, method_name="m9_rec", pattern=Pattern.sinogram)
    method10 = make_test_method(mocker, method_name="data_checker", pattern=Pattern.sinogram)
    method11 = make_test_method(mocker, method_name="calculate_stats", pattern=Pattern.all)
    p = Pipeline(loader=loader, methods=[method1, method2, method3, method4, method5, method6, method7, method8, method9, method10, method11])
    t = TaskRunner(p, reslice_dir=tmp_path, comm=MPI.COMM_WORLD)
    ...
    for ind in range(0, sections_number):
        assert t._get_methods_name_for_snapshot(_sections[ind]) == METHODS_NAMES_EXPECTED[ind]
