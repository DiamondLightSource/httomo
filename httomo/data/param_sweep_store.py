...
from httomo.sweep_runner.param_sweep_store_interfaces import (
    ParamSweepSink,
    ParamSweepSource,
)
...
class ParamSweepReaderHdf5(ParamSweepSource):
    """
    Read parameter sweep results from hdf5 file produced by writer, extracting middle slices
    along the `detector_y` dim
    """

    def __init__(self, source: ParamSweepWriterHdf5) -> None:
        self._no_of_sweeps = source.no_of_sweeps
        self._extract_dim: Literal[1] = 1
        self._single_shape = source.single_shape
        self._total_shape = source.total_shape
        assert (
            source._dataset is not None
        ), "Reader should be created from writer with dataset not `None`"
        self._dataset = source._dataset
        self._aux_data = source.aux_data
        self._h5file = source._h5file
        self.filepath: Path = source.filepath
    ...
    def read_sweep_results(self) -> ParamSweepBlock:
        ...
        slices[self.extract_dim] = detector_y_slice
        return ParamSweepBlock(
            data=self._dataset[slices[0], slices[1], slices[2]],
            aux_data=self.aux_data,
            slicing_dim=self.extract_dim,
        )
...
class ParamSweepWriter:
    ...
    # TODO: Add params for temppath and backing enum, like DataSetStoreWriter, and switch
    # between RAM and hdf5 file within a single class, rather than using separate classes which
    # have the majority of the same code
...
class ParamSweepWriterHdf5(ParamSweepSink):
    """Write parameter sweep results, concatenating them along the `detector_y` dim"""

    def __init__(self, no_of_sweeps: int, filepath: Path) -> None:
        self._concat_dim: Literal[1] = 1
        self._no_of_sweeps = no_of_sweeps
        self._no_of_sweeps_written: int = 0
        self._single_shape: Optional[Tuple[int, int, int]] = None
    ...
    def make_reader(self) -> ParamSweepReaderHdf5:
        if self._dataset is None:
            raise ValueError("Cannot make reader when no data has been written yet")
        return ParamSweepReaderHdf5(self)
    ...
    def write_sweep_result(self, block: ParamSweepBlock):
        ...
        if self._dataset is None:
            ...
            self._slices_per_sweep = self.single_shape[self.concat_dim]
            self._dataset = self._h5file.create_dataset(
        ...
        self._dataset[slices[0], slices[1], slices[2]] = block.data
        self.increment_no_of_sweeps_written()
